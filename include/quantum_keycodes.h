...
#define C_S_T(kc) \
  MT(MOD_LCTL | MOD_LSFT, kc)  // Left Control + Shift e.g. for gnome-terminal
#define MEH_T(kc)                    \
  MT(MOD_LCTL | MOD_LSFT | MOD_LALT, \
     kc)  // Meh is a less hyper version of the Hyper key -- doesn't include
          // GUI, so just Left Control + Shift + Alt
#define LCAG_T(kc) \
...
// Dedicated keycode versions for Hyper and Meh, if you want to use them as
// standalone keys rather than mod-tap
