...
void BleKeyboard::move(int8_t x, int8_t y, int8_t wheel, int8_t hWheel) {
  uint8_t m[5];
  m[0] = this->_buttons;
  m[1] = x;
  m[2] = y;
...
void BleKeyboard::pressMouse(uint8_t b) { buttons(_buttons | b); }
void BleKeyboard::releaseMouse(uint8_t b) { buttons(_buttons & ~b); }
...
bool BleMouse::isConnected(void) { return this->connected; }
